...
sim_cosp = np.zeros((10755, 10755))
for v1 in range(L_mat_.shape[0]):
    for v2 in range(L_mat_.shape[0]):
        sim_cosp[v1][v2] = L_mat_[v1][v2] / math.sqrt(L_mat_[v1][v1] * L_mat_[v2][v2])

# 转换为字典
All_dict = {}
for i in range(1, sim_cosp.shape[0]):
    similarList = list(sim_cosp[i])
    List = {}
