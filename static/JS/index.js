...
window.onload = async function() {
  let fourData = await sendAJAX('http://127.0.0.1:5000/overview-data');

 
  let staNums= document.querySelectorAll('.sta-num');
  let num = 0;
  let graphData = {nodes:[],edges:[]};
  let scholarUrl = 'http://127.0.0.1:5000/';
...
searchBtn.onclick = ()=>{
  $.post(scholarUrl+'api-login-verify',(res)=>{  
    // let legal = /^([1-9])[0-9]{0,4}$/g;
    let legal = /^[0-9]*$/
    if(!legal.test(search.value)){
      alert('请输入数字！');
    }else{

      if(parseInt(search.value)>10755 || parseInt(search.value)<=0){
        alert('请输入范围内的数字！');
      }else{
        if($.isEmptyObject(res)){
          window.open(`/${search.value}/page`);
        }
        else{
            alert('请先登录再进行此操作！');
...
userBtn[1].onclick = ()=>{
  ...
  const getNode = function(){
    let url = 'http://127.0.0.1:5000/api-shown_nodes-data';
    $.get(url,(res)=>{
      let nodeTemp = [];
      let data = eval(res);
  ...
  const getNodeDetail = function(id){
    let url = 'http://127.0.0.1:5000/api-id-data';
...
// 第二个统计图
        let useData1 = await sendAJAX('http://127.0.0.1:5000/excellent_school');
    ...
    renderChart2 = ()=>{
        $.get('http://127.0.0.1:5000/excellent_scholar',useData2=>{
            let getData2 = [];
            useData2 = JSON.parse(useData2);
